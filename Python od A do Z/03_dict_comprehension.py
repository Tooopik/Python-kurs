...
stocks_type = {key:  'Corp'if 'Corp' in val else 'Inc' for (
    key, val) in stocks.items()}
...
numbers_dict2 = {key: key**2 for key in numbers if key % 2 == 0}
...
# %%
nested_dict2 = {'001': {'price': 100, 'items': 4},
...
# %%
{key: val['price'] for(key, val) in nested_dict2.items()}
{key: val['price']*val['items'] for(key, val) in nested_dict2.items()}
